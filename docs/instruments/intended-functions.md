...
| Autoflight annunciation | Planned advisory surface reporting what the automation holds and what it is armed to hold; no command guidance credit | Autoflight engagement, modes, and targets under [`AIR-IN-015`](requirements.md#air-in-015), with the altitude datum under [`AIR-IN-002`](requirements.md#air-in-002) | [`AIR-OUT-003`](requirements.md#air-out-003); conventional and degraded/reversionary modes | Missing modes annunciate nothing rather than the last known ones; a stale or failed group wears its own flag under [`AIR-OUT-004`](requirements.md#air-out-004) | Reporting what an autopilot says it is doing establishes no autopilot, flight-guidance, or command-guidance function; a mode this display cannot name fails its group rather than annunciating a neighbour |
