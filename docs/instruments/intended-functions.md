...
| Composed multi-panel surface | Planned surface presenting several panels at once, each in a declared region of a logical screen; placement only, adding no instrument of its own | The panels composed onto it, resolved from one snapshot and one alert state per composed frame | [`AIR-OUT-011`](requirements.md#air-out-011); every panel's own modes are unchanged by being composed | A panel's failure is presented inside its own region and suppresses no other panel, under [`AIR-HAZ-004`](requirements.md#air-haz-004) | Co-locating panels on one surface establishes no independence: every slot resolves from the same snapshot, so a composed six-pack is one function drawn several times, not several sources ([`AIR-HAZ-009`](requirements.md#air-haz-009)) |
| Autoflight annunciation | Planned advisory surface reporting what the automation holds and what it is armed to hold; no command guidance credit | Autoflight engagement, modes, and targets under [`AIR-IN-015`](requirements.md#air-in-015), with the altitude datum under [`AIR-IN-002`](requirements.md#air-in-002) | Mode annunciation and target readouts only; no flight-data mode participates. No `AIR-OUT` requirement covers an autoflight annunciator, so this row cites none rather than borrowing one written for another surface | Missing modes annunciate nothing rather than the last known ones; a stale or failed group wears its own flag under [`AIR-OUT-004`](requirements.md#air-out-004) | Reporting what an autopilot says it is doing establishes no autopilot, flight-guidance, or command-guidance function; a mode this display cannot name fails its group rather than annunciating a neighbour |
| Airframe configuration | Planned comparison and reversion surface reporting what the airframe is set to; no primary-flight credit | Sensed flap position and elevator trim under [`AIR-IN-016`](requirements.md#air-in-016), with source trust under [`AIR-IN-009`](requirements.md#air-in-009). The group carries aileron and rudder trim, which this surface does not draw | Configuration scales only; no flight-data mode participates. No `AIR-OUT` requirement covers a configuration surface, so this row cites none rather than borrowing one written for another surface | An absent sensor draws dashes, never a centred pointer; a value outside its declared range fails the group under [`AIR-OUT-004`](requirements.md#air-out-004) | Reporting a sensed surface position establishes no flight-control, configuration-warning, or takeoff-configuration function; a selected detent is never shown in place of a sensed position |
| Machine-monitoring readout | Planned advisory text surface for non-flight status (build stamps, link health, payload state) | Bounded, charset-restricted text channel under [`AIR-IN-014`](requirements.md#air-in-014) | Advisory text rows only; no flight-data mode participates | Malformed or stale content fails the channel visibly; the readout never poses as a flight instrument | Machine text is advisory, arrives on its own freshness policy, and implies no flight-data validity |
