...
use indicate_instrument_scene::validate_layers;
...
fn golden_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../indicate-instrument-scene/corpus/scene-conformance-corpus.json")
}
...
fn golden_matches_reference() {
    ...
    let on_disk = std::fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!(
            "cannot read {}: {e}. Regenerate with REGEN_CONFORMANCE_CORPUS=1 cargo test -p indicate-instrument-raster",
            path.display()
        )
    });
...
fn resource_budgets_flip_verdict_at_their_limits() {
    ...
    assert!(matches!(
        ...
        Err(indicate_instrument_scene::LayerError::StackOverCapacity { .. })
    ));
