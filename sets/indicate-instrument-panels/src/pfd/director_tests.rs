...
use indicate_instrument_scene::{Cmd, PaintMode, SceneCmds, SceneWriter};
...
fn a_stale_command_disappears_rather_than_freezing() {
    ...
    data.director.status = indicate_instrument_state::SignalStatus::Failed;
    let scene = render(&data);
    assert_eq!(guidance_fills(&scene), 0);
}
...
fn bars_need_a_shown_attitude() {
    ...
    data.pitch_rad.status = indicate_instrument_state::SignalStatus::Missing;
    let scene = render(&data);
    assert_eq!(guidance_fills(&scene), 0);
}
...
fn the_unusual_attitude_tier_strips_the_bars() {
    ...
    let mut data = with_director(FdEngagement::Engaged, Some(60.0));
    data.roll_rad = indicate_instrument_state::Sig::with_status(
        2.8,
        indicate_instrument_state::SignalStatus::Valid,
    );
    data.pitch_rad = indicate_instrument_state::Sig::with_status(
        -0.9,
        indicate_instrument_state::SignalStatus::Valid,
    );
